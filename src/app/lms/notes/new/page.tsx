...
export default function LmsNewNotePage() {
  ...
  const canSave = title.trim().length > 2 && body.trim().length > 4;
  ...
  return (
    <div className="space-y-8">
      <div className="min-w-0">
        ...
        <p className={LMS_PAGE_SUBTITLE}>Frontend-only note creation (mock). Saved in local session state.</p>
      </div>
      ...
      <div className={`${LMS_CARD_CLASS} space-y-4`}>
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-2">
          ...
          <LmsCtaButton variant="secondary" onClick={() => router.push('/lms/notes')}>
            Cancel
          </LmsCtaButton>
        </div>
      </div>
