...
import { Award, CheckCircle2, XCircle, ArrowRight, ArrowLeft } from 'lucide-react';
    ...
    return (
      <div className="space-y-8 pb-8">
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-gray-50 border border-gray-100 rounded-3xl">
          <p className="text-gray-500 font-medium">Session attempt could not be verified or the quiz does not exist.</p>
          <div className="mt-6 flex flex-col sm:flex-row items-center gap-4">
            <Link href={`/lms/quizzes/${quizId}/attempt`} className="inline-flex items-center justify-center rounded-xl bg-white border border-gray-200 px-5 py-2.5 text-sm font-semibold text-gray-900 transition-all hover:bg-gray-50">
              Retake quiz
            </Link>
            <Link href="/lms/quizzes" className="inline-flex items-center justify-center rounded-xl bg-[#28A8E1] px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[#208bc0]">
              Dashboard
            </Link>
          </div>
        </div>
    ...
    <div className="space-y-6">
      <div className={`${LMS_CARD_CLASS} border-violet-100 bg-violet-50/20`}>
        <div className="flex items-start gap-3">
          <Award className="h-5 w-5 text-violet-700 mt-0.5" strokeWidth={2} aria-hidden />
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold uppercase tracking-wide text-gray-400">Score</p>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-gray-600">
              ...
              <span className="text-gray-300">·</span>
              <LmsStatusBadge label={score >= 70 ? 'Good' : 'Needs work'} tone={score >= 70 ? 'success' : 'warning'} />
            </div>
            {score < 100 && breakdown.correct < questions.length && (
              <p className="mt-3 text-sm text-gray-700 bg-white/50 px-3 py-2 border border-violet-100 rounded-lg inline-block">
                Weak areas: {breakdown.rows.filter(r => !r.ok).map(r => r.q.prompt.substring(0, 20) + '...').join(' / ')}
              </p>
            )}
          </div>
          <Link
      ...
      <div className="space-y-3">
        ...
        <ul className="space-y-3">
          {breakdown.rows.map(({ q, chosen, ok }) => (
            <li key={q.id} className={`${LMS_CARD_CLASS} ${ok ? 'border-emerald-100' : 'border-rose-100'}`}>
              <div className="flex items-start gap-3">
                {ok ? (
                  <CheckCircle2 className="h-5 w-5 text-emerald-700 mt-0.5" strokeWidth={2} aria-hidden />
                ) : (
                  <XCircle className="h-5 w-5 text-rose-700 mt-0.5" strokeWidth={2} aria-hidden />
                )}
                <div className="min-w-0 flex-1">
                  ...
                  {!ok ? (
                    <p className="mt-1 text-sm text-gray-600">
                      Correct:{' '}
                      <span className="font-semibold text-emerald-800">{q.options[q.correctIndex]}</span>
                    </p>
                  ) : null}
                  <p className="mt-3 text-sm font-normal text-gray-500">{q.explanation}</p>
                </div>
              </div>
            </li>
          ))}
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => {
            const label = score < 100 ? `Review missed quiz topics: ${quiz.title}` : `Review: ${quiz.title}`;
            addPlannedItem({ id: `quiz:${quizId}:review-${Date.now()}`, type: 'quiz', label, href: `/lms/quizzes/${quizId}/attempt` });
            toast.push({ title: 'Added to plan', message: 'Quiz review added to Career Path plan.', tone: 'success' });
          }}
        >
