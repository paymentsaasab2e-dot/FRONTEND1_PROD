...
export default function LmsQuizResultPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="space-y-8">
      <div className="min-w-0">
        ...
        <h1 className="mt-3 text-3xl sm:text-4xl font-bold text-gray-900 tracking-tight">Results</h1>
        <p className={LMS_PAGE_SUBTITLE}>Review your answers (frontend-only mock).</p>
      </div>

      <QuizResultClient quizId={params.id} />
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-4 flex flex-col sm:flex-row gap-2">
                <Link
                  href={`/lms/quizzes/${lmsQuizIdForSkill(quiz?.skill ?? 'react')}/attempt?skill=${quiz?.skill ?? 'react'}`}
