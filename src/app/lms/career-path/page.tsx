...
export default function LmsCareerPathPage() {
  ...
  const missionSteps = useMemo(() => careerMissionRoadmap.flatMap((phase) => phase.steps), []);
  ...
  const dynamicNextStep = hasStarted
    ? nextIncompleteStep
      ? `Next tracked milestone: ${nextIncompleteStep.label}`
  ...
  const handleReset = () => {
    ...
    careerReset();
    toast.push({ title: 'Path reset', message: 'Career-path tracking has been cleared.', tone: 'info' });
  };
      ...
      <section className="space-y-4 rounded-2xl border border-violet-100 bg-white/70 p-5 shadow-sm transition-shadow duration-200 hover:shadow-md sm:p-6">
        ...
        <div className={`space-y-4 ${!hasStarted ? 'opacity-60 grayscale-[20%]' : ''}`}>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {careerMissionRoadmap.map((phase) => {
              const phaseCompletedSteps = phase.steps.filter((step) => trackedCompletedStepIds.includes(step.id));
                    ...
                    {phase.steps.map((step) => {
                      ...
                      return (
                        <li key={step.id}>
                          <button
                            type="button"
                            onClick={() => openStepTarget(step.href)}
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1">
                              <span className={`block ${stepDone ? 'font-semibold text-gray-900' : ''}`}>{step.label}</span>
                              <span className="mt-1 block text-xs font-medium text-gray-500">
                                {stepDone ? 'Completed from LMS activity' : hasStarted ? 'Open target' : 'Start the path to unlock tracking'}
                              </span>
                            </span>
                          </button>
                        </li>
        ...
        <AIInsightCard
          ...
          ctaLabel={nextIncompleteStep?.href ? 'Open next milestone' : 'Launch quizzes'}
          onCta={() => openStepTarget(nextIncompleteStep?.href ?? '/lms/quizzes')}
        />
                  ...
                  <div className="mt-2 flex shrink-0 items-center gap-2 opacity-100 transition-opacity sm:mt-0 sm:opacity-0 sm:group-hover:opacity-100">
                    <button
                      ...
                      onClick={() => {
                        ...
                        toast.push({ title: 'Item removed', message: 'Removed from your plan.', tone: 'info' });
                      }}
      ...
      <section className="space-y-4 pt-4">
        ...
        <div className={LMS_CARD_CLASS}>
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between md:gap-1">
            {careerMissionRoadmap.map((phase, index) => {
              const phaseCompletedSteps = phase.steps.filter((step) => trackedCompletedStepIds.includes(step.id));
              const isPhaseDone = phase.steps.length > 0 && phaseCompletedSteps.length === phase.steps.length;
              ...
              return (
                <Fragment key={phase.id}>
                  ...
                  {index < careerMissionRoadmap.length - 1 ? (
